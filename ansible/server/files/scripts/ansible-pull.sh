...
then
  ...
  mkdir -p ${ansible_dir}
  [[ -e "${ansible_dir}/install-ansible.sh" ]] || curl -L 'https://tangled.org/heywoodlh.io/nixos-configs/raw/main/ansible/server/files/scripts/install-ansible.sh' -o ${ansible_dir}/install-ansible.sh
  ...
  curl --silent -L https://tangled.org/heywoodlh.io/nixos-configs/raw/main/ansible/requirements.yml -o ${ansible_dir}/requirements.yml
  ...
  ansible-pull -U "${repo}" ansible/server/standalone.yml "${EXTRA_ARGS}"
fi
