            ...
            users.heywoodlh = { ... }: {
              ...
              home.packages = let
                nixos-switch = pkgs.writeShellScriptBin "nixos-switch" ''
                  ${pkgs.lima}/bin/limactl shell nixos nixos-rebuild switch --flake $HOME/opt/nixos-configs#nixos-lima
                '';
                nixos-build = pkgs.writeShellScriptBin "nixos-build" ''
                  ${pkgs.lima}/bin/limactl shell nixos nixos-rebuild build --flake $HOME/opt/nixos-configs#nixos-lima
                '';
              in [
                nixos-switch
                nixos-build
