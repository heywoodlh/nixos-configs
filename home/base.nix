...
let
  ...
  nixos-configs-test = pkgs.writeShellScriptBin "nixos-configs.sh" ''
    ...
    then
      for server in "nixos-gaming.barn-banana.ts.net" "nixos-m1-mac-mini.barn-banana.ts.net"
      do
  ...
  '';
  tangledSync = pkgs.callPackage ../pkgs/tangled-sync.nix {};
  tangledSyncWrapper = pkgs.writeShellScriptBin "tangled-sync.sh" ''
    ${tangledSync}/bin/tangled-sync.sh
